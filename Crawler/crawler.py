...
from config import BASE_URL, HEADERS, SEED_CATEGORIES, PAGE_LIMIT
from utils import slugify
from category import discover_subcategories
from article import get_article_detail
from storage import save_article

def crawl_category(category_slug):
    for page in range(1, PAGE_LIMIT + 1):
        url = f"{BASE_URL}/{category_slug}?page={page}"
        res = requests.get(url, headers=HEADERS, timeout=10)
        soup = BeautifulSoup(res.text, "html.parser")

        links = soup.select("a.title, h3 a")
        if not links:
            break
        ...
        for link in links:
            ...
            article = {
                ...
                "author": detail["author"],
                "category": {
                    "parent": category_slug.split("/")[0],
                    "child": category_slug.split("/")[-1],
                    "slug": category_slug
                },
                "source": "bongdaplus",
                "article_url": article_url,
...
def run():
    for seed in SEED_CATEGORIES:
        subs = discover_subcategories(seed)
        for cat in subs:
